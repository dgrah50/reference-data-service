"""Integration tests for Bitget adapter."""
...
@pytest.mark.asyncio
async def test_bitget_fetch_spot_snapshot(integration_timeout):
    """Test fetching real spot data from Bitget."""
    # Note: Bitget uses same endpoints for mainnet and testnet
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_bitget_fetch_perpetual_snapshot(integration_timeout):
    """Test fetching real perpetual data from Bitget."""
    # Note: Bitget uses same endpoints for mainnet and testnet
    adapter = BitgetExchangeAdapter(is_testnet=False)
    try:
        # Get the endpoint configuration
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_bitget_fetch_futures_snapshot(integration_timeout):
    """Test fetching real futures data from Bitget."""
    # Note: Bitget uses same endpoints for mainnet and testnet
    adapter = BitgetExchangeAdapter(is_testnet=False)
    try:
        # Get the endpoint configuration
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_bitget_adapter_initialization(integration_timeout):
    """Test that Bitget adapter initializes correctly."""
    adapter = BitgetExchangeAdapter(is_testnet=False)
    try:
