...
@pytest.mark.asyncio
async def test_binance_fetch_spot_snapshot(integration_timeout):
    """Test fetching real spot data from Binance testnet."""
    adapter = BinanceExchangeAdapter(is_testnet=True)
    try:
        # Get the endpoint configuration
        endpoint = adapter.config.get_endpoint(InstrumentType.SPOT, is_testnet=True)
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_binance_fetch_perpetual_snapshot(integration_timeout):
    """Test fetching real perpetual data from Binance testnet."""
    adapter = BinanceExchangeAdapter(is_testnet=True)
    try:
        # Get the endpoint configuration
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_binance_fetch_futures_snapshot(integration_timeout):
    """Test fetching real futures data from Binance testnet."""
    adapter = BinanceExchangeAdapter(is_testnet=True)
    try:
        # Get the endpoint configuration
        endpoint = adapter.config.get_endpoint(InstrumentType.FUTURES, is_testnet=True)
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_binance_adapter_initialization(integration_timeout):
    """Test that Binance adapter initializes correctly in testnet mode."""
    adapter = BinanceExchangeAdapter(is_testnet=True)
    try:
